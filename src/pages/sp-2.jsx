...
const C = {
  ...
  // ── Broadcast masthead palette (Hero + ticker only) ──────────────
  ink:         "#0b0d16",   // studio-dark background
  ink2:        "#161a2b",   // slightly lifted panel on ink
  inkBorder:   "#272c42",
  crimson:     "#d1293d",   // breaking / live / on-air accent
  crimsonDark: "#a31f30",
  paper:       "#f5f2ea",   // warm paper tone for stat strip
  onInk:       "#f5f3ee",   // primary text on dark
  onInk2:      "#8d90ab",   // secondary text on dark
};
...
// ─────────────────────────────────────────────────────────────────
// GLOBAL STYLE
// Loads the display/mono font pair and shared keyframes used by
// the ticker, the live-dot pulse, and the REC timecode.
// ─────────────────────────────────────────────────────────────────
function GlobalStyle() {
  return (
    <style>{`
      @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,600;0,700;0,800;1,600&family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500&display=swap');
      ...
      @keyframes livePulse {
        0%, 100% { opacity: 1; box-shadow: 0 0 0 0 rgba(209,41,61,0.55); }
        50%      { opacity: 0.55; box-shadow: 0 0 0 6px rgba(209,41,61,0); }
      }
    `}</style>
  );
}
...
// Sits directly under the nav, on the ink-dark masthead.
// Purely decorative/atmospheric — duplicated once so the CSS
// animation can loop seamlessly.
// ─────────────────────────────────────────────────────────────────
function Ticker({ lang }) {
  const items = [
    t(lang, { en: "Parliament budget debate enters third day", np: "संसद्मा बजेट बहस तेस्रो दिनमा प्रवेश" }),
    t(lang, { en: "Flood relief reaches Terai districts", np: "तराई जिल्लामा बाढी राहत पुग्यो" }),
    t(lang, { en: "New episode airs Thursday, 8 PM on NTV", np: "नयाँ एपिसोड बिहीबार बेलुका ८ बजे NTV मा" }),
    t(lang, { en: "Gold trade investigation: part two out now", np: "सुन व्यापार अनुसन्धान: दोस्रो भाग प्रकाशित" }),
  ];
  const line = items.join("     ●     ");

  return (
    <div style={{ background: C.ink2, borderBottom: `1px solid ${C.inkBorder}`, overflow: "hidden" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", height: 34, gap: 12 }}>
        <span style={{
          flexShrink: 0, display: "flex", alignItems: "center", gap: 6,
          fontSize: 10, fontWeight: 700, letterSpacing: 1, color: C.crimson,
          fontFamily: "'JetBrains Mono', monospace", textTransform: "uppercase",
        }}>
          <span style={{ width: 6, height: 6, borderRadius: "50%", background: C.crimson, display: "inline-block", animation: "livePulse 1.6s infinite" }} />
          {t(lang, { en: "Breaking", np: "ताजा" })}
        </span>
        <div style={{ flex: 1, overflow: "hidden", maskImage: "linear-gradient(90deg, transparent, #000 24px, #000 calc(100% - 24px), transparent)" }}>
          <div style={{
            display: "inline-flex", whiteSpace: "nowrap",
            fontSize: 12, color: C.onInk2, fontFamily: "'JetBrains Mono', monospace",
...
function Navbar({ lang, setLang, isLoggedIn, onLoginClick, onLogout }) {
  ...
  // Unscrolled = sitting on top of the dark hero, so nav reads light-on-dark.
  // Scrolled = sitting on the paper-light body, so it flips to the normal theme.
  const linkColor = scrolled ? C.text2 : C.onInk2;
  const nameColor = scrolled ? C.text : C.onInk;

  return (
    <nav style={{
      position: "sticky", top: 0, zIndex: 100,
      background: scrolled ? "rgba(248,247,251,0.95)" : "rgba(11,13,22,0.75)",
      backdropFilter: "blur(12px)",
      borderBottom: `1px solid ${scrolled ? C.border : C.inkBorder}`,
      transition: "all 0.2s",
    }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", display: "flex", alignItems: "center", height: 62, gap: 16 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          <div style={{ width: 38, height: 38, borderRadius: "50%", background: C.crimson, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Noto Sans Devanagari', serif", fontSize: 14, fontWeight: 600 }}>
            स
          </div>
          <div>
            <div style={{ fontSize: 14, fontWeight: 700, color: nameColor, transition: "color 0.2s" }}>Santosh Pokharel</div>
        ...
        <div style={{ display: "flex", gap: 2, marginLeft: "auto" }}>
          {["news","blog","videos","thoughts","about","contact"].map(id => (
            <button key={id} onClick={() => scroll(id)} style={{ background: "none", border: "none", padding: "6px 12px", borderRadius: 6, fontSize: 13, color: linkColor, cursor: "pointer", fontFamily: "inherit", transition: "color 0.2s" }}>
        ...
        <button
          ...
          style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 6, border: `1px solid ${scrolled ? C.border : C.inkBorder}`, background: scrolled ? C.surface : C.ink2, fontSize: 12, fontWeight: 600, color: linkColor, cursor: "pointer", fontFamily: "inherit", transition: "all 0.2s" }}
        >
        ...
        {isLoggedIn ? (
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button onClick={onLogout} style={{ ...S.outlineBtn, padding: "6px 12px", fontSize: 12, color: linkColor, borderColor: scrolled ? C.border : C.inkBorder }}>Logout</button>
          </div>
        ) : (
...
// ─────────────────────────────────────────────────────────────────
// REC TIMECODE
// Small live-updating counter used inside the camera-frame photo.
// Purely atmospheric — reinforces "this is a live broadcast feed."
// ─────────────────────────────────────────────────────────────────
function RecTimecode() {
  ...
  const h = fmt(Math.floor(secs / 3600));
  const m = fmt(Math.floor((secs % 3600) / 60));
  const s = fmt(secs % 60);
  return (
    <div style={{
      position: "absolute", top: 10, left: 10, display: "flex", alignItems: "center", gap: 5,
      background: "rgba(11,13,22,0.55)", backdropFilter: "blur(4px)", borderRadius: 4,
      padding: "3px 7px",
    }}>
      <span style={{ width: 6, height: 6, borderRadius: "50%", background: C.crimson, display: "inline-block", animation: "livePulse 1.6s infinite" }} />
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#fff", letterSpacing: 0.5 }}>
        REC {h}:{m}:{s}
      </span>
    </div>
  );
...
// ─────────────────────────────────────────────────────────────────
// HERO SECTION — broadcast masthead
// Dark studio background, camera-frame photo with a live timecode,
// and a serif display name — the page opens like a broadcast feed
// rather than a SaaS landing card.
// ─────────────────────────────────────────────────────────────────
function Hero() {
  const lang = useLang();

  return (
    <section id="home" style={{ background: C.ink, position: "relative", overflow: "hidden" }}>

      {/* faint vignette / studio-light glow behind the photo */}
      <div style={{
        position: "absolute", top: "-20%", right: "8%", width: 420, height: 420, borderRadius: "50%",
        background: `radial-gradient(circle, rgba(209,41,61,0.14) 0%, rgba(209,41,61,0) 70%)`,
        pointerEvents: "none",
      }} />

      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", position: "relative" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1.15fr 0.85fr", minHeight: 520, gap: 48, alignItems: "center" }}>

          {/* LEFT — identity */}
          <div style={{ padding: "64px 0", display: "flex", flexDirection: "column", gap: 22 }}>

            {/* On-air badge */}
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11, fontWeight: 700, color: C.crimson, letterSpacing: 1.5, textTransform: "uppercase", fontFamily: "'JetBrains Mono', monospace" }}>
              <span style={{ width: 8, height: 8, borderRadius: "50%", background: C.crimson, animation: "livePulse 1.6s infinite", display: "inline-block" }} />
              {t(lang, { en: "On Air · NTV Nepal", np: "प्रसारणमा · NTV नेपाल" })}
            </div>
            ...
            <div>
              <h1 style={{
                ...
                fontSize: 52, lineHeight: 1.05, color: C.onInk, letterSpacing: -0.5,
              }}>
                Santosh Pokharel
              </h1>
              <div style={{ fontFamily: "'Noto Sans Devanagari', sans-serif", fontSize: 21, color: C.onInk2, marginTop: 8 }}>
                सन्तोष पोखरेल
              </div>
              <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
                <span style={{ ...S.tag, background: "rgba(209,41,61,0.15)", color: "#ff8b98" }}>
                  {t(lang, { en: "Senior Reporter", np: "वरिष्ठ पत्रकार" })}
                </span>
                <span style={{ ...S.tag, background: "rgba(201,154,63,0.16)", color: "#e3bb6a" }}>
                  {t(lang, { en: "8+ Years", np: "८+ वर्ष" })}
                </span>
              </div>
            </div>

            {/* Bio */}
            <p style={{ fontSize: 15.5, color: C.onInk2, lineHeight: 1.75, maxWidth: 440 }}>
              {t(lang, {
            ...
            {/* Stats — on a paper-toned strip so they read as a printed press card, not a UI widget */}
            <div style={{
              display: "flex", gap: 0, background: C.ink2, border: `1px solid ${C.inkBorder}`,
              borderRadius: 10, overflow: "hidden", width: "fit-content",
            }}>
              {[["8+","Years"],["2,100+","Reports"],["45k","YT Subs"],["77","Districts"]].map(([n, l], i) => (
                <div key={l} style={{ padding: "12px 20px", borderLeft: i === 0 ? "none" : `1px solid ${C.inkBorder}` }}>
                  <div style={{ fontSize: 19, fontWeight: 700, color: C.onInk, fontFamily: "'JetBrains Mono', monospace" }}>{n}</div>
                  <div style={{ fontSize: 10.5, color: C.onInk2, marginTop: 3, textTransform: "uppercase", letterSpacing: 0.4 }}>{l}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
              <button
                style={{ ...S.accentBtn, background: C.crimson, boxShadow: "0 8px 24px -8px rgba(209,41,61,0.6)" }}
              ...
              <button
                style={{ ...S.outlineBtn, color: C.onInk, borderColor: C.inkBorder, background: "transparent" }}
          ...
          {/* RIGHT — camera-frame photo */}
          <div style={{ display: "flex", justifyContent: "center" }}>
            <div style={{ position: "relative", width: 300, aspectRatio: "4 / 5" }}>

              {/* Photo — replace this div with <img src="your-photo.jpg" /> */}
              <div style={{
                width: "100%", height: "100%", borderRadius: 4, position: "relative", overflow: "hidden",
                background: `linear-gradient(150deg, #22263a 0%, ${C.ink2} 100%)`,
                display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
                fontSize: 44, gap: 8,
              }}>
                <span>🎙️</span>
                {/* ↑ REPLACE THIS WHOLE DIV's background/emoji with:
                    <img
                      src="/santosh.jpg"
                      alt="Santosh Pokharel"
                      style={{ width: "100%", height: "100%", objectFit: "cover" }}
                    />
                    (keep the corner brackets + RecTimecode as overlays on top of it)
                */}
                <span style={{ fontSize: 10, color: C.onInk2, fontWeight: 600, letterSpacing: 0.5, fontFamily: "'JetBrains Mono', monospace" }}>ADD PHOTO</span>
              ...
              {[
                ...
                <div key={i} style={{
                  position: "absolute", width: 22, height: 22, borderColor: C.crimson, borderStyle: "solid",
                  ...pos,
                }} />
              ))}
            </div>
        ...
        {/* Latest coverage strip */}
        <div id="news" style={{ borderTop: `1px solid ${C.inkBorder}`, padding: "28px 0 40px" }}>
          <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 1.5, color: C.crimson, textTransform: "uppercase", display: "flex", alignItems: "center", gap: 6, marginBottom: 16, fontFamily: "'JetBrains Mono', monospace" }}>
            <span style={{ width: 6, height: 6, borderRadius: "50%", background: C.crimson, display: "inline-block" }} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
            {[
              ...
              <div key={i} style={{ background: C.ink2, border: `1px solid ${C.inkBorder}`, borderRadius: 10, padding: "14px 16px", cursor: "pointer", transition: "border-color 0.15s" }}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                  <span style={{ ...S.tag, background: "rgba(209,41,61,0.15)", color: "#ff8b98" }}>{item.tag}</span>
                </div>
                <div style={{ fontSize: 13, fontWeight: 500, lineHeight: 1.55, color: C.onInk, fontFamily: lang === "np" ? "'Noto Sans Devanagari', sans-serif" : "inherit" }}>
                  {item.title}
                </div>
                <div style={{ fontSize: 11, color: C.onInk2, marginTop: 5 }}>🕐 {item.time} · NTV Nepal</div>
              </div>
            ))}
          </div>
        </div>
